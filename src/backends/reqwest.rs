...
impl ReqwestClient {
    ...
    /// Consumes the builder and returns a functional `ReqwestOperation`.
    /// Automatically adapts to single URLs, `PresignedDownloads`, or `PresignedUploads`.
    #[must_use]
    pub fn op(&self, target: impl Into<ReqwestTarget>) -> ReqwestOperation {
        ReqwestOperation {
...
#[derive(Clone)]
pub enum ReqwestTarget {
    Single(String),
    Download(super::PresignedDownload),
    Upload(super::PresignedUpload),
}

// Allow passing a single raw URL
impl From<String> for ReqwestTarget {
    fn from(url: String) -> Self {
        Self::Single(url)
    }
}

// Allow passing the exact Download struct from the SdkClient
impl From<super::PresignedDownload> for ReqwestTarget {
    fn from(bundle: super::PresignedDownload) -> Self {
        Self::Download(bundle)
    }
}

// Allow passing the exact Upload struct from the SdkClient
impl From<super::PresignedUpload> for ReqwestTarget {
    fn from(bundle: super::PresignedUpload) -> Self {
        Self::Upload(bundle)
    }
}
...
impl ObjectOperation for ReqwestOperation {
    ...
    async fn get_size(&mut self) -> Result<u64> {
        match &self.target {
            ReqwestTarget::Download(bundle) => Ok(bundle.total_size),
            ReqwestTarget::Upload(_) => Err(Error::Context {
                context: "Cannot calculate download size of an Upload target.",
                source: None,
            }),
            ReqwestTarget::Single(url) => {
                let url = url.clone();
        ...
        I: Into<S3Payload> + MaybeSend + 'static,
    {
        ...
        match &self.target {
            ReqwestTarget::Download(_) => Err(Error::Context {
                context: "Cannot perform PUT operation on a Download target.",
                source: None,
            }),
            ...
            ReqwestTarget::Upload(bundle) => {
                let mut etags = Vec::new();
                let mut upload_tasks = futures::stream::FuturesUnordered::new();
                let mut current_chunk = bytes::BytesMut::new();

                let mut part_iter = bundle.parts.clone().into_iter();
                let mut next_part = part_iter.next();

                info!("Starting mapped presigned multipart upload pooling");
                ...
                loop {
                    ...
                    let Some(chunk_res) = chunk_opt else { break }; // Stream exhausted
                    ...
                    while let Some(part) = &next_part {
                        let expected_size = part.expected_size as usize;

                        if current_chunk.len() >= expected_size {
                            let payload = current_chunk.split_to(expected_size).freeze();
                            ...
                            while upload_tasks.len() >= self.multipart_concurrency {
                                tokio::select! {
                                    res = upload_tasks.next() => {
                                        if let Some(res) = res {
                                            let (p_num, etag) = res.ctx("Task panic")??;
                                            etags.push((p_num, etag));
                                        }
                                    },
                                    () = self.cancel_token.cancelled() => return Err(Error::Cancelled),
                                }
                            ...
                            // Buffer doesn't contain enough data for the current part yet
                            break;
                        }
                    }
                }

                // If the stream ends but there's a final part, flush the remaining buffer.
                // S3 dictates that the final chunk can be smaller than standard chunk bounds.
                if !current_chunk.is_empty() {
                    let part =
                        next_part.ctx("Stream contained data but no presigned URL parts remain")?;
        ...
        W: S3Writer,
    {
        ...
        match &self.target {
            ReqwestTarget::Upload(_) => Err(Error::Context {
                context: "Cannot perform GET operation on an Upload target.",
                source: None,
            }),
            ...
            ReqwestTarget::Download(bundle) => {
                writer.size_hint(bundle.total_size).await?;
                ...
                let download_tasks = bundle.parts.clone().into_iter().map(|part| {
                            ...
                            || {
                                ...
                                async move {
                                    ...
                                    while let Some(chunk_result) = stream.next().await {
                                        let chunk =
                                            chunk_result.ctx("Failed to read stream chunk")?;
                ...
                Ok(S3Return {
                    read_length: total_written,
                    object_size: Some(bundle.total_size),
                })
            }
        }
    }
